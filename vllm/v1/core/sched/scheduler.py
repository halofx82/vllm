...
from vllm.v1.spec_decode.asymspec.verifier_bridge import (
    ...
    arm_asymspec_diagnostic_next_spec_tokens,
...
class Scheduler(SchedulerInterface):
    ...
    def update_from_output(
        ...
        for req_id, num_tokens_scheduled in num_scheduled_tokens.items():
            ...
            if req_id in asymspec_live_capture_complete and is_asymspec_live_capture:
                # The worker has already persisted all diagnostic rows for
                # this disposable request. Finish through the scheduler's
                # regular request/block cleanup path, before generic
                # speculative acceptance bookkeeping sees its K=2 pair.
                if request.spec_token_ids:
                    raise RuntimeError(
                        "AsymSpec live capture completed before V1 consumed "
                        "spec tokens."
                    )
            ...
            if new_token_ids:
                ...
                if req_id in model_runner_output.asymspec_fixed_acceptance_counts:
                    # The fixed result itself used ordinary V1 speculative
                    # bookkeeping.  Optionally arm one follow-up pair only
                    # for the target-state diagnostic, after R has been
                    # appended as the next canonical-but-uncomputed seed.
                    arm_asymspec_diagnostic_next_spec_tokens(
                        request, self.vllm_config.speculative_config
                    )
                else:
                ...
                # The pair is generated by the AsymSpec-local draft runtime
                # after V1 samples its canonical-but-uncomputed seed.  This
                # merely installs it in the existing V1 request field; the
                # following scheduler pass remains entirely generic.
                # A live pair exists only on the bootstrap sampler result.
                # Fixed-outcome diagnostics deliberately return ordinary V1
                # speculative output without producing another FULL pair.
