...
def build_asymspec_fixed_acceptance_outcome(
    *,
    ...
    if (spec_decode_metadata.target_logits_indices.numel() != 2
    ...
    *,
    ...
    if writer:
        ...
        capture = {
            ...
            f"{row_prefix}t0": logits[int(target_indices[0])].detach().cpu().to(
                torch.bfloat16
            ),
            f"{row_prefix}t1": logits[int(target_indices[1])].detach().cpu().to(
                torch.bfloat16
            ),
            f"{row_prefix}t2": logits[int(bonus_indices[0])].detach().cpu().to(
                torch.bfloat16
            ),
        }
        ...
        if path.exists():
            ...
            if (
                not is_follow_up
                and tuple(existing.get("candidate_token_ids", ())) != tuple(tokens)
            ):
        ...
        if has_pending_fixed_outcome:
            capture.setdefault("fixed_chain_rows", []).append({
                ...
                "t0": logits[int(target_indices[0])].detach().cpu().to(
                    torch.bfloat16
                ),
                "t1": logits[int(target_indices[1])].detach().cpu().to(
                    torch.bfloat16
                ),
                "t2": logits[int(bonus_indices[0])].detach().cpu().to(
                    torch.bfloat16
                ),
            })
        torch.save(capture, path)
